...
AuDHDMAP 0.8.0 is a private, single-owner application by default. It is suitable for a trusted individual or household behind BoxPilot, a private network, a VPN, or a carefully configured HTTPS reverse proxy. It is not a multi-tenant collaboration service and does not implement per-map roles or server-side user isolation.

The separate `AUDHDMAP_PUBLIC_DEMO=1` deployment mode does not provide a server workspace. Each tab uses browser `sessionStorage`, which is removed when that tab closes. The server workspace routes fail with `403`. Map export accepts one bounded workspace payload, returns a no-store file response, and does not persist the payload. The demo is not encrypted storage and should not be used for private or durable notes.
...
- Public-demo mode does not create login sessions or initialize `/data`. Workspace, complete backup, restore, import, recovery-point, attachment, permanent-delete, and authenticated export routes fail before request-body parsing or filesystem-heavy work. Only the dedicated bounded map-export request accepts demo workspace data.
- Browser-demo data is schema-bounded before use, limited to 8 MiB in `sessionStorage`, revision checked within a tab, and reset to checked-in sample data if the stored record is malformed.
- The dedicated demo export route normalizes the supplied workspace, applies the same map and PDF size limits as private export, returns `private, no-store`, and never calls the workspace store.
...
- PDF scope is capped at 1,000 active thoughts before document rendering so a single request cannot force hundreds of buffered outline pages into a small container. Branch Focus narrows the scope without changing the map.
...
- The production stage upgrades Alpine's OpenSSL runtime libraries and removes the unused npm CLI and its transitive packages before dropping to the unprivileged application user.
